...
@bot.tree.command(name="пополнить", description="Добавить заработанные суммы (обязательно прикрепите 2-5 скриншотов)")
...
async def пополнить(interaction: discord.Interaction, дата: str, суммы: str):
    ...
    if len(attachments) < 2 or len(attachments) > 5:
        await interaction.response.send_message("❌ Прикрепите от 2 до 5 скриншотов.", ephemeral=True)
        return
    ...
    embed.add_field(
        name="🖼 Скриншоты",
        value=f"Приложено файлов: {len(attachments)}",
        inline=False
    )
